...
export class TorboxFormatter extends BaseFormatter {
  constructor() {
    super({
      name: `
{stream.proxied::istrue["🕵️‍♂️ "||""]}{addon.name}{stream.inLibrary::istrue[" (Your Media) "||""]}{service.cached::istrue[" (Instant "||""]}{service.cached::isfalse[" ("||""]}{service.id::exists["{service.shortName})"||""]}{stream.resolution::exists[" ({stream.resolution})"||""]}
      `,
      description: `
